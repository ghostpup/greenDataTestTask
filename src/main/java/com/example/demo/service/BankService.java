...
public class BankService {
    ...
    //Методы CRUD
    public BankEntity create(BankEntity bankEntity) throws BankIdentifierAlreadyExists, BankNameAlreadyExists, BankIdentifierWrongFormat, BankEntityParameterIsNull {
    ...
    public BankEntity update(BankEntity bankEntity) throws BankIdentifierWrongFormat, BankIdentifierAndNameExistsCombine, BankEntityParameterIsNull {
    ...
    public Iterable<BankEntity> getBanks(String criteriaName, String criteriaValue,
                                        String orderBy, String orderDirection) throws ClientSortOrFilterParamsNotValid, BankSortOrFilterParamsNotValid {
        ...
        if((!criteriaName.isEmpty() && !Arrays.asList(fields).contains(criteriaName)) ||
           (!orderBy.isEmpty() && !Arrays.asList(fields).contains(orderBy))){
            throw new BankSortOrFilterParamsNotValid("Неверные названия параметров фильтрации и сортировки. Доступные значения: "+ Arrays.toString(fields));
        }
    ...
    private Boolean checkUniqueBankIdentifier(String bankIdentifier){
        if(bankRepo.findByBankIdentifier(bankIdentifier) != null)
            return false;
        return true;
    }

    private Boolean checkFormatBankIdentifier(String bankIdentifier){
        if(!bankIdentifier.matches("[0-9]+") || bankIdentifier.length() != 9)
            return false;
        return true;
    }
