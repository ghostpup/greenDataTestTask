...
import com.example.demo.exception.client.ClientHasDeposit;
...
import com.example.demo.exception.client.ClientOrgFormIsNotValid;
...
public class ClientService {
  ...
  public ClientEntity create(@RequestBody ClientEntity clientEntity)
      throws ClientParameterIsNull, ClientNameAlreadyExists, ClientAdressAlreadyOccupied, ClientOrgFormIsNotValid {
  ...
  public ClientEntity update(@RequestBody ClientEntity clientEntity)
      throws ClientParameterIsNull, ClientNameAndAdressAlreadyExistsCombined, ClientOrgFormIsNotValid {
    if (clientEntity.getName() == null) {
      throw new ClientParameterIsNull("Значение параметра name равно null. Ожидалась строка.");
    }
    if (clientEntity.getShortName() == null) {
  ...
  public Iterable<ClientEntity> getClients(String criteriaName, String criteriaValue,
    ...
    if ((!criteriaName.isEmpty() && !Arrays.asList(fields).contains(criteriaName)) ||
      ...
      throw new ClientSortOrFilterParamsNotValid("Неверные названия параметров фильтрации и сортировки." +
