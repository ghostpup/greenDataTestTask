...
import com.example.demo.exception.deposit.DepositSortOrFilterParamsNotValid;
...
public class DepositService {
  ...
  public DepositEntity create(@RequestBody DepositEntity depositEntity) throws DepositParameterIsNull,
                                                                               DepositClientIdentifierNotFound,
                                                                               DepositBankIdentifierNotFound,
                                                                               DepositMonthTermNotValid,
                                                                               DepositPercentageNotValid {
  ...
  public DepositEntity update(@RequestBody DepositEntity depositEntity) throws DepositPercentageNotValid,
                                                                               DepositBankIdentifierNotFound,
                                                                               DepositMonthTermNotValid,
  ...
  public Iterable<DepositEntity> getDeposits(String criteriaName, String criteriaValue,
      ...
      throws DepositSortOrFilterParamsNotValid {
    ...
    if ((!criteriaName.isEmpty() && !Arrays.asList(fields).contains(criteriaName)) ||
      ...
      throw new DepositSortOrFilterParamsNotValid(
          "Неверные названия параметров фильтрации и сортировки. Доступные значения: " + Arrays.toString(fields));
    }
    return depositRepo.findAndSortByCriteria(criteriaName, criteriaValue, orderBy, orderDirection);
  }

  private void checkAllRulesForAllParams(DepositEntity depositEntity) throws DepositParameterIsNull,
                                                                             DepositBankIdentifierNotFound,
                                                                             DepositClientIdentifierNotFound,
                                                                             DepositMonthTermNotValid,
                                                                             DepositPercentageNotValid {
    ...
    if (depositEntity.getMonthTerm() < 0) {
      throw new DepositMonthTermNotValid(
          "Указанный monthTerm имеет неверный формат." +
          " Значение должно быть больше нуля, если вклад имеет срок, или равно нулю," +
          " если вклад бессрочный и иметь тип Long");
    }
    if (depositEntity.getPercentage() <= 0) {
      throw new DepositPercentageNotValid("Указанный percentage имеет неверный формат." +
                                          " Значение должно быть больше нуля и иметь тип Long");
    }
    if (bankRepo.findById(depositEntity.getBankIdentifier()).isEmpty()) {
